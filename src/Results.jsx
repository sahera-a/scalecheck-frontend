...
function getHealthInfo(score) {
  ...
  if (score >= 60) return { label: "Good", className: "health-good" };
  if (score >= 40) return { label: "At Risk", className: "health-atrisk" };
...
function Results({ result, onRestart, onFeedback }) {
  ...
  const health = getHealthInfo(overall_score);
  ...
  return (
    <div className="results">
      ...
      <div className="results-actions">
        <button className="cta-button" onClick={() => generateReportPdf(result)}>
          Download PDF Report
        </button>
